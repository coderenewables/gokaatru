...
def _mapping_for_sensor(sensor: str) -> dict[str, str | None]:
    """Find the height-level sensor mapping entry associated with a named speed sensor."""
    for sensor_map in session.sensor_mapping.values():
...
def _apply_icing_filter(df: pd.DataFrame, sensor: str, mask: np.ndarray, params: dict[str, object]) -> int:
    """Apply an icing filter where zero standard deviation and low temperature indicate frozen instrumentation."""
    sensor_map = _mapping_for_sensor(sensor)
...
def _apply_tower_shadow(df: pd.DataFrame, sensor: str, mask: np.ndarray, params: dict[str, object]) -> int:
...
def _apply_timestamp_gap_fill(df: pd.DataFrame, start_date: str, end_date: str) -> int:
    ...
    session.timeseries_df = df.reindex(full_index)
    inserted = session.timeseries_df.index.difference(before_index)
    inserted_mask = (
...
def _apply_rule(rule_type: str, sensor: str, params: dict[str, object], start_date: str, end_date: str) -> int:
...
def apply_cleaning_rule(rule_type: str, sensor: str, params: str, start_date: str = "", end_date: str = "") -> dict:
    ...
    records_affected = _apply_rule(rule_type, sensor, parsed, start_date, end_date)
    ...
    session.cleaning_log.append(entry)
...
def get_cleaning_log() -> dict:
    ...
    return {"entries": [entry.copy() for entry in session.cleaning_log]}


@mcp.tool()
def undo_cleaning_rule(entry_index: int) -> dict:
    """Restore raw data and replay all cleaning steps except the selected log entry."""
    if session.raw_timeseries_df is None:
        raise ValueError("Raw timeseries backup is not available")
    if entry_index < 0 or entry_index >= len(session.cleaning_log):
        raise ValueError(f"Cleaning log entry_index out of range: {entry_index}")
    retained = [entry.copy() for idx, entry in enumerate(session.cleaning_log) if idx != entry_index]
    session.timeseries_df = session.raw_timeseries_df.copy(deep=True)
    ...
    session.cleaning_log = retained
    return {"status": "ok", "remaining_rules": len(session.cleaning_log)}
