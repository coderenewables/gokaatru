...
def _require_series(sensor_name: str) -> pd.Series:
    ...
    return session.timeseries_df[sensor_name]
...
def _align_pair(sensor_a: str, sensor_b: str) -> pd.DataFrame:
    ...
    pair = pd.concat([_require_series(sensor_a), _require_series(sensor_b)], axis=1, join="inner")
...
def compute_diurnal_profile(sensor_name: str) -> dict:
    ...
    hourly = _require_series(sensor_name).groupby(_require_series(sensor_name).index.hour).mean().reindex(range(24))
    return {
        "sensor": sensor_name,
        "hours": list(range(24)),
