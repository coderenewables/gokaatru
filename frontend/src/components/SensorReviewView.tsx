...
function TurbulenceSection({ speedSensor, directionSensor }: { speedSensor: string; directionSensor: string }) {
  ...
  return <><section className="path-panel"><h3>Turbulence intensity statistics</h3><dl className="metrics-grid"><MetricTile label="Mean TI" value={summary.mean_ti.toFixed(3)} /><MetricTile label="P90 TI" value={summary.p90_ti.toFixed(3)} /><MetricTile label="Representative TI" value={summary.representative_ti.toFixed(3)} /><MetricTile label="IEC TI near 15 m/s" value={summary.iec_ti_at_15ms.toFixed(3)} /></dl><Disclosure source={summary} title="What this turbulence figure assumes" /></section><section className="path-panel"><h3>TI versus wind speed</h3><PlotFrame plotName="turbulence_intensity" params={{ speed_sensor: speedSensor, sensor_b: summary.sd_sensor }} height={500} /></section>{directionSensor ? <section className="path-panel"><h3>Turbulence wind rose</h3><PlotFrame plotName="turbulence_windrose" params={{ speed_sensor: speedSensor, sensor_b: summary.sd_sensor, direction_sensor: directionSensor }} height={500} /></section> : <UnavailableSection title="Turbulence wind rose" requirement="A wind-direction sensor is required to summarize turbulence intensity by sector." />}</>;
}
...
function AdvancedChecksSection({ speedSensor }: { speedSensor: string }) {
  ...
  return <>
    {results.extremes ? <><section className="path-panel"><h3>Extreme winds</h3><dl className="metrics-grid"><MetricTile label="Annual maxima" value={String(results.extremes.sample_years)} />{results.extremes.gev.wind_50_year !== undefined ? <MetricTile label="GEV 50-year" value={`${results.extremes.gev.wind_50_year.toFixed(2)} m/s`} /> : null}{results.extremes.gev.wind_100_year !== undefined ? <MetricTile label="GEV 100-year" value={`${results.extremes.gev.wind_100_year.toFixed(2)} m/s`} /> : null}<MetricTile label="Gumbel 50-year" value={`${results.extremes.gumbel.wind_50_year.toFixed(2)} m/s`} /></dl><Disclosure source={results.extremes} title="What this extreme wind assumes" collapsedByDefault={false} /></section><section className="path-panel"><h3>Annual maxima &amp; return levels</h3><PlotFrame plotName="extremes_fit" params={{ speed_sensor: speedSensor }} height={420} /></section></> : <UnavailableSection title="Extreme winds" requirement="At least two calendar years with valid observations are required for a screening fit." />}
